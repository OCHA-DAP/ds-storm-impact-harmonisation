# 16 · Three-source storm-exposure source comparison
...
| `panel.py`, `analysis.py` | earlier exploratory stats pass (coverage, pairwise log-ratios → `out/*`) |
...
```bash
python build.py                       # rebuild the workbook from the canonical blob diagnostic (fast)
python build.py --refresh-diagnostic  # re-run the ~15-20 min GDACS/ADAM probe first, re-upload to blob
```
...
## Early findings (exploratory `analysis.py` pass)
...
When stable, a `book/NN-source-distribution.qmd` chapter can read the workbook /
`out/*` directly. `fm_matching.py`'s upstream-vs-vendored copies must be kept in
sync (the ds-storms-alerts copy differs only by a provenance docstring).
