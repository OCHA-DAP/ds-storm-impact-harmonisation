...
2. **GDACS produces a usable *per-country* exposure for only ~60 storms.** GDACS
   ...
   gaps, not zeros** — the per-country comparison is genuinely thin (~60 storms),
   and the old "fake-zero" reading of those NULLs is what this chapter corrects.
3. **Where comparable, CHD reads lower — except in subnational aggregate.** On
   both-positive units CHD runs systematically below GDACS (median CHD/GDACS
   ≈ 0.61 adm0, 0.41 adm1), and at **adm0 that holds in aggregate too (0.52×)**.
   At **adm1 CHD's broader subnational footprint flips the aggregate (1.51×)** —
   it assigns exposure to many subnational units GDACS leaves at a true 0; a real
   coverage difference, not a fake-zero artefact. The sources also disagree on
   *who* is exposed about as often as not (κ ≈ 0).
...
CHD is our own NHC DB, so a missing value is a true 0. The **per-country**
...
storms), and the old reading that treated those `-1`s as zeros manufactured a
large fake zero-gap.
...
#| label: fig-fourcell
#| code-fold: true
#| fig-cap: "Four-cell split of the adm0 overlap per pair (GDACS cells now 0-vs-NaN per the five-case rule). For CHD vs GDACS the *defensible* zero-gap (CHD>0, GDACS true 0) is now modest — about the size of the both-positive set, not the dominant cell it appeared to be when GDACS's -1 values were fake-zeroed."
...
Of the **484** CHD-vs-GDACS adm0 comparable cells (both non-NaN), **199 are
both-positive**, **84 are CHD-positive while GDACS is a true 0** (case 2/3 — GDACS
computed the storm but didn't place this country in its footprint), and 63 the
reverse. These are *defensible* zeros, not the old 442 fake zeros — those came
from treating GDACS's `-1` "not computed" values as 0, and are now correctly NaN.
...
gaps excluded. Putting the true 0s back lets us ask two questions the magnitude
lens can't: do the sources agree on *who* is exposed, and on the *total*? Both
sources are summed/compared over exactly the same cells (a NaN on either side
drops the cell for both). First, the comparable set on **symlog** axes so the
true 0s are visible:
...
#| label: fig-aggregate
#| code-fold: true
#| fig-cap: "Aggregate over the comparable set, split into both-positive vs the defensible zero-gap. On both-positive units GDACS is higher (CHD/GDACS ≈ 0.49 adm0, 0.59 adm1). At adm0 the small zero-gap leaves CHD's total below GDACS (0.52×); at adm1 the larger zero-gap — CHD's broader subnational footprint, all defensible true 0s — lifts CHD's total above GDACS (1.51×). A real coverage difference, not a fake-zero artefact."
...
for ax, (panel, lvl) in zip(axes, ((panel0, "adm0"), (panel1, "adm1"))):
    ...
    ax.set_title(f"{lvl}: both-positive CHD/GDACS={bp_ratio:.2f}, "
                 ...
                 ("zero-gap lifts CHD above GDACS" if tot_ratio > 1
                  else "CHD stays below GDACS"), fontsize=9.5)
    ax.legend(fontsize=8, frameon=False)
fig.suptitle("Aggregate: CHD below GDACS at adm0, above at adm1 "
             "(broader subnational footprint)", fontsize=12, fontweight="bold")
fig.tight_layout()
plt.show()
```

The full comparison reframes the headline honestly. Per shared unit CHD runs
~30–60% *below* GDACS, and at **adm0 that holds in aggregate too** (total CHD/GDACS
≈ **0.52**). At **adm1 the direction flips to CHD-higher** (≈ **1.51**) — but this
time it is **not a fake-zero artefact**; it is a real coverage difference, CHD
assigning exposure to many subnational units GDACS computed as a true 0. The
sources also **agree on who is even exposed only about half the time** (κ ≈ 0).
*Neither lens alone is complete:* both-positive says "CHD is the systematic low
outlier"; the full view adds "CHD's broader subnational footprint lifts its adm1
total above GDACS, and the two disagree on presence as often as not."
...
- **The per-country comparison is thin (~60 storms).** GDACS's `getimpact`
  ...
  era). Those are **NaN per country, not zeros** — treating the `-1`s as 0 (the
  old approach) manufactured a large fake zero-gap and a spurious aggregate flip.
...
not the fake zeros the old reading implied. Where comparable, CHD reads lower per
unit; in aggregate CHD is below GDACS at adm0 (0.52×) but above at adm1 (1.51×,
its broader subnational footprint); and the two agree on *who* is exposed only
about half the time (κ ≈ 0).**
